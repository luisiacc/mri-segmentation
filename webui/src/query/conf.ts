...
axiosQuery.post = (url: string, data: DataType) => axiosQuery({ method: 'post', url: url, config: { data } })

axiosQuery.put = (url: string, data: DataType) => axiosQuery({ method: 'put', url: url, config: { data } })

axiosQuery.patch = (url: string, data: DataType) => axiosQuery({ method: 'patch', url: url, config: { data } })

axiosQuery.delete = (url: string) => axiosQuery({ method: 'delete', url: url })

const defaultQueryFn = async ({ queryKey }: QueryFunctionContext<QueryKey, any>) => {
